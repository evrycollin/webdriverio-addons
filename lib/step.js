...
function Step(title) {
    ...
    this.tools = require('webdriverio-addons').tools;
    ...
    Step.prototype.attachLog = function( message, detail ) {
      process.send({ event: 'test:log', cid: browser.cid, message: message, detail: detail, uuid: this.uuid });
      return this;
    };
     
    Step.prototype.attachPng = function( title, file ) {
      process.send({ event: 'test:attach', cid: browser.cid, title: title, type: 'image/png', file: file, uuid: this.uuid });
      return this;
    };
    
    Step.prototype.attachFile = function( title, file, type ) {
      process.send({ event: 'test:attach', cid: browser.cid, title: title, type: type, file: file, uuid: this.uuid });
      return this;
    };
    ...
    Step.prototype.done = function( res, message, detail ) {
      ...
      process.send( { event: 'step:end', cid: browser.cid, status: this.result, uuid: this.uuid } );
    ...
    Step.prototype.ko = function( message, detail ) {
      ...
      //this.tools.takeFullScreenshot('Error screenshot test ko : '+(message!=null ? message : ''), this);
      this.done('failed', message, detail);
    };    
    Step.prototype.error = function( message, detail ) {
      this.tools.takeFullScreenshot('Error screenshot test error : '+(message!=null ? message : ''), this);
    ...
    Step.prototype.assertVisual = function( refScreenshot, test, message ) {
      try {
        ...
        this.tools.takeFullScreenshot('Error assertVisual : '+refScreenshot+' '+(message!=null ? message : ''), this);
        this.ko(message, err);
        throw err;
      }
      return this;
    ...
    Step.prototype.scroll = function( selector, message ) {
      this.log( 
        'scroll : '+( message!=null ? message : selector ), 
        { selector:selector } 
      );
      try {
        browser.scroll(selector);
      } catch(err) {
        var newErr = new Error('cannot scroll on not found element : '+selector,err);
        this.ko(message, newErr);
        throw newErr;
      }
      return this;
    ...
    Step.prototype.takeFullScreenshot = function(title) {
      this.tools.takeFullScreenshot(title, this);
      return this;
    };    
    ...
    process.send( { event: 'step:start', cid: browser.cid, title: title, uuid: this.uuid } );
