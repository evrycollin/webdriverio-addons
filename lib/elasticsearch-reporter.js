...
var ElasticseachReporter = function ( baseReporter, config) {
    ...
    this.storeToElasticSearch = function( type, document, uuid ) {
      ...
      url += _this.options.elasticseachReporter[type+'Index'] + '/' + _this.options.elasticseachReporter[type+'Type'] + '/' + uuid;
      console.log('es-storing',type, uuid);
   ...
   this.on('runner:start', function (event) {
    ...
    this.on('test:attach', function(event) {
        ...
        //attachment.content = fs.readFileSync(attachment.file, {encoding: 'base64'} );
